...
bonobo::mesh_data
parametric_shapes::createCrosshair(float const width,
	float const height)
{
	auto vertices = std::vector<glm::vec3>(4);
	vertices[0] = glm::vec3(width, 0, 0);
	vertices[1] = glm::vec3(-width, 0, 0);
	vertices[2] = glm::vec3(0, height, 0);
	vertices[3] = glm::vec3(0, -height, 0);

	auto indices = std::vector<glm::vec2>(2);
	indices[0] = glm::uvec2(0, 1);
	indices[1] = glm::uvec2(2, 3);
	...
	data.indices_nb = indices.size() * 2u;
	glGenBuffers(1, &data.ibo);
	assert(data.ibo != 0u);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, data.ibo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<GLsizeiptr>(indices.size() * sizeof(glm::uvec2)), reinterpret_cast<GLvoid const*>(indices.data()), GL_STATIC_DRAW);
...
bonobo::mesh_data
parametric_shapes::createAsteroid(float const radius,
	unsigned int const longitude_split_count,
	unsigned int const latitude_split_count,
	float speed,
	glm::vec3 moveDirection)
{
	...
	auto tangents = std::vector<glm::vec3>(vertices_nb);
	auto binormals = std::vector<glm::vec3>(vertices_nb);
	auto speedVec = std::vector<float>(vertices_nb);
	auto direction = std::vector<glm::vec3>(vertices_nb);
	...
	auto const binormals_offset = tangents_offset + tangents_size;
	auto const binormals_size = static_cast<GLsizeiptr>(binormals.size() * sizeof(glm::vec3));
	auto const speed_offset = binormals_offset + binormals_size;
	auto const speed_size = static_cast<GLsizeiptr>(speedVec.size() * sizeof(float));
	auto const direction_offset = speed_offset + speed_size;
	auto const direction_size = static_cast<GLsizeiptr>(direction.size() * sizeof(glm::vec3));
	auto const bo_size = static_cast<GLsizeiptr>(vertices_size
		+ normals_size
		+ texcoords_size
		+ tangents_size
	...
	glBufferSubData(GL_ARRAY_BUFFER, speed_offset, speed_size, static_cast<GLvoid const*>(speedVec.data()));
	glEnableVertexAttribArray(5U);
	glVertexAttribPointer(5U, 1, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<GLvoid const*>(speed_offset));

	glBufferSubData(GL_ARRAY_BUFFER, direction_offset, direction_size, static_cast<GLvoid const*>(direction.data()));
	glEnableVertexAttribArray(static_cast<unsigned int>(6U));
	glVertexAttribPointer(6U, 3, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<GLvoid const*>(direction_offset));
